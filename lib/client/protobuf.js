...
(function (exports, global){
  ...
  function encodeProp(value, type, offset, buffer, protos){
    ...
    if(type.indexOf('map<') == 0  && type.indexOf('>', type.length -1) != -1) {
        var pairs = type.substr(4, type.length-5).split(',');
        if(pairs.length == 2) {
            var keys = Object.keys(value);
            offset = writeBytes(buffer, offset, codec.encodeUInt32(keys.length));
            keys.forEach(function (key) {
                offset = encodeProp(key, pairs[0], offset, buffer, protos);
                offset = encodeProp(value[key], pairs[1], offset, buffer, protos);
            });
            return offset;
        }
    }
  ...
  function decodeProp(type, protos){
      if(type.indexOf('map<') == 0  && type.indexOf('>', type.length -1) != -1) {
          ...
          if(pairs.length == 2) {
              ...
              for(var i=0; i<length; i++) {
                  var key = decodeProp(pairs[0], protos);
                  obj[key] = decodeProp(pairs[1], protos);
              }
              return obj;
          }
      }
      ...
      switch(type){
      case 'bool':
          return codec.decodeUInt32(getBytes()) == 1;
      case 'uInt32':
